...
uses
  Classes, ExtCtrls, Forms, Windows, uWVBrowser,
  uWVTypeLibrary, uWVTypes, uWVWindowParent;
...
type
  ...
  private
    ...
    procedure BrowserAcceleratorKeyPressed(Sender: TObject;
...
uses
  Controls, LCLIntf, LCLType, Markdown_Renderer, SysUtils,
  uWVCoreWebView2Args, uWVLoader;
...
begin
  ...
  GlobalWebView2Loader.UserDataFolder := UTF8Decode(
    IncludeTrailingPathDelimiter(GetAppConfigDir(False)) + 'webview2');
...
procedure TPreviewForm.BrowserInitializationError(Sender: TObject;
  ErrorCode: HRESULT; const ErrorMessage: wvstring);
begin
  ShowWebViewError(UTF8Encode(ErrorMessage));
end;

procedure TPreviewForm.BrowserAcceleratorKeyPressed(Sender: TObject;
  const Controller: ICoreWebView2Controller;
  const Args: ICoreWebView2AcceleratorKeyPressedEventArgs);
var
...
begin
  ...
  ErrorShown := True;
  InitializationTimer.Enabled := False;
  ErrorText := 'Não foi possível abrir a visualização HTML.' + LineEnding +
    'Instale ou atualize o Microsoft Edge WebView2 Runtime.';
  if Details <> '' then
    ErrorText := ErrorText + LineEnding + LineEnding + Details;
  LCLIntf.MessageBox(Handle, PChar(ErrorText),
    'Visualização indisponível', MB_OK or MB_ICONERROR);
  ModalResult := mrCancel;
end;

...
function AddMenuItem(Parent: TMenuItem; const Caption: string;
  Shortcut: TShortCut; Handler: TNotifyEvent): TMenuItem;
begin
...
begin
  ...
  AddMenuItem(FileMenu, '&Novo', ShortCut(Ord('N'), [ssCtrl]),
    Actions.NewDocument);
  AddMenuItem(FileMenu, '&Abrir...', ShortCut(Ord('O'), [ssCtrl]),
    Actions.OpenDocument);
  AddMenuItem(FileMenu, '&Salvar', ShortCut(Ord('S'), [ssCtrl]),
    Actions.SaveDocument);
  AddMenuItem(FileMenu, 'Salvar &como...',
    ShortCut(Ord('S'), [ssCtrl, ssShift]), Actions.SaveDocumentAs);
  AddMenuItem(FileMenu, '&Exportar HTML', ShortCut(VK_F2, []),
    Actions.ExportHtml);
  AddMenuItem(FileMenu, 'Exportar HTML &como...',
    ShortCut(VK_F2, [ssCtrl]), Actions.ExportHtmlAs);
